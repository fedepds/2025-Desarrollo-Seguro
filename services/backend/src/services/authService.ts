...
class AuthService {
  ...
  static async createUser(user: User) {
    ...
    const invite_token_expires = new Date(Date.now() + INVITE_TTL);
      ...
      .insert({
        ...
        //  Se almacena la contraseña en texto plano. Almacenamiento inseguro.
        //   Se usara bcrypt para cifrar la contraseña, ya que es ideal para esto.
  ...
  static async updateUser(user: User) {
    ...
    if (user.password) {
        updatedPassword = await bcrypt.hash(user.password, SALT_ROUNDS);
    }
